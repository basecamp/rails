...
called [Lexxy](https://github.com/basecamp/lexxy), which is used in web
applications to provide users with a user-friendly interface for creating and
editing rich text content. It handles everything from providing enriching
capabilities like the formatting of text, adding links or quotes, embedding
images, and much much more. See [the Lexxy documentation](https://lexxy.dev/docs/)
for examples.

Applications upgraded from a version of Rails before 8.2 keep using
[Trix](https://trix-editor.org/), the editor Action Text included previously,
which is still supported. See [Choosing an Editor](#choosing-an-editor).
...
inconsistencies.<br><br> Lexxy, which is built on Meta's
[Lexical](https://lexical.dev) framework, and Trix both sidestep these
inconsistencies by treating `contenteditable` as an I/O device: when input makes
its way to the editor, the editor converts that input into an editing operation
on its internal document model, then re-renders that document back into the
editor. This gives the editor complete control over what happens after every
keystroke and avoids the need to use `execCommand` and the inconsistencies that
come along with it.
...
- Creates `actiontext.css`, which imports Lexxy's styles.
...
update your views to make use of the rich text editor for that field. To
do so, use a
...
This will display the editor, which provides the functionality to create and
update your rich text accordingly. Later we'll go into details about [how to
update the styles for the
editor](action_text_overview.html#removing-or-adding-editor-styles).
...
## Customizing the Rich Text Content Editor
...
Action Text renders the editor named by `config.action_text.editor`. New
applications use Lexxy. Applications that call `config.load_defaults` with a
version before 8.2 use Trix, and can keep it after moving to 8.2 by setting the
editor explicitly:
...
with one editor can be edited with the other. See [Upgrading from Rails 8.1 to
Rails 8.2](upgrading_ruby_on_rails.html#lexxy-is-the-default-action-text-editor)
for switching an existing application to Lexxy.

### Removing or Adding Editor Styles

By default, Action Text will render rich text content inside an element with the
editor's content class, `.lexxy-content` for Lexxy or `.trix-content` for Trix.
This is set in `app/views/layouts/action_text/contents/_content.html.erb`.
Elements with this class are then styled by the editor's stylesheet.

If you’d like to update the editor's styles, you can add your custom styles in
`app/assets/stylesheets/actiontext.css`. For Lexxy, that file imports Lexxy's
stylesheet, and [the Lexxy documentation](https://lexxy.dev/docs/) describes how
to customize it. For Trix, it includes both the full set of styles for Trix and
the overrides needed for Action Text.
...
When using Trix, Action Text dispatches [Active Storage Direct Upload
Events](active_storage_overview.html#direct-upload-javascript-events) during the
File attachment lifecycle. Lexxy uploads files itself and dispatches its own
`lexxy:upload-start`, `lexxy:upload-progress`, and `lexxy:upload-end` events on
the `<lexxy-editor>` element, described in [the Lexxy
documentation](https://lexxy.dev/docs/).
